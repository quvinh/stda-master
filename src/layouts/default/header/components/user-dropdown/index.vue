<template>
  <Dropdown placement="bottomLeft" :overlayClassName="`${prefixCls}-dropdown-overlay`">
    <span :class="[prefixCls, `${prefixCls}--${theme}`]" class="flex">
      <img :class="`${prefixCls}__header`" :src="getUserInfo.photo" />
...
<script lang="ts" setup>
  ...
  import headerImg from '@/assets/images/user.png';
  import { propTypes } from '@/utils/propTypes';
  // import { openWindow } from '@/utils';
  import { createAsyncComponent } from '@/utils/factory/createAsyncComponent';
  import { useGo } from '@/hooks/web/usePage';

  type MenuEvent = 'logout' | 'doc' | 'lock' | 'api' | 'oi';
  ...
  const getUserInfo = computed(() => {
    const { name = '', photo: p, desc } = userStore.getUserInfo || {};
    let photo = headerImg;
    if (p) {
      const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://127.0.0.1:8000';
      photo = `${BACKEND_URL}/media/${p}`;
    }
    return { name, photo, desc };
  });
