...
export function useTableScroll(
  ...
  function getMarginPaddingHeight() {
    ...
    return (
      tableWrapperPadding + 8 // 来自于 .vben-basic-table-form-container 或是 .p-4
    );
  }
  ...
  async function calcTableHeight() {
    ...
    let paginationHeight = caclPaginationHeight(tableEl);
    let footerHeight = caclFooterHeight(tableEl);
    ...
    if (tableData.length === 0) {
      const antTableEmpty: any = tableEl.querySelector('.ant-table-expanded-row-fixed');
      if (antTableEmpty)
        antTableEmpty?.style.setProperty('height', `${height - 16}px`, 'important');
    }
