<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <UnitSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      ...
      >
        <template #extra>
          <Space>
            <Button size="small" :href="`${BACKEND_URL}/excel/Units.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-units`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import { deleteUnitApi, exportExcelData, searchUnitApi } from '@/api/sys/unit';
  ...
  import UnitModal from './component/UnitModal.vue';
  import UnitSidebar from './component/UnitSidebar.vue';
  ...
  async function fetchUnit(payload: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchUnitApi({
        page: page.value,
      ...
      let totalRecord = units.value.length;
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteUnitApi(record?.id);
      if (result) {
        fetchUnit();
