<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <StandardParameterSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      <Card
        ...
        size="small"
        title="Quản lý thông số tiêu chuẩn"
      >
        <template #extra>
          <Space>
...
<script setup lang="ts">
  ...
  import {
    deleteStandardParameterApi,
    exportExcelDataStandardParameterApi,
    searchStandardParameterApi,
  ...
  import StandardParameterModal from './component/StandardParameterModal.vue';
  import StandardParameterSidebar from './component/StandardParameterSidebar.vue';
  ...
  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchStardParameter({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchStardParameter({ page: page.value, pageSize: size });
  }

  async function fetchStardParameter(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchStandardParameterApi({
        page: page.value,
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteStandardParameterApi(record?.id);
      if (result) {
        fetchStardParameter();
  ...
  async function exportData() {
    try {
      const response: any = await exportExcelDataStandardParameterApi();
