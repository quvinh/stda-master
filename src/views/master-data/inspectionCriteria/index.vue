<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <InspectionCriteriaSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      ...
      >
        <template #extra>
          <Space>
            <Button size="small" :href="`${BACKEND_URL}/excel/InspectionCriterias.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-inspection-criterias`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import {
    deleteInspectionCriteriaApi,
    exportInspectionCriteriaApi,
    searchInspectionCriteriaApi,
  ...
  import InspectionCriteriaModal from './component/InspectionCriteriaModal.vue';
  import InspectionCriteriaSidebar from './component/InspectionCriteriaSidebar.vue';

  const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://127.0.0.1:8000';

  const inspectionCriterias = ref<IInspectionCriteria[]>([]);
  ...
  const [registerTable, { setPagination }] = useTable({
    dataSource: inspectionCriterias,
  ...
  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchInspectionCriteria({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchInspectionCriteria({ page: page.value, pageSize: size });
  }

  async function fetchInspectionCriteria(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchInspectionCriteriaApi({
        page: page.value,
      ...
      if (response?.data) {
        // Set data
        inspectionCriterias.value = response.data ?? [];
        console.log(inspectionCriterias.value);

        // Set pagination
        const pagination: IPagination = response?.pagination ?? null;
        let totalRecord = inspectionCriterias.value.length;
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteInspectionCriteriaApi(record?.id);
      if (result) {
        fetchInspectionCriteria();
  ...
  async function handleExport(params: any) {
    try {
      const response = await exportInspectionCriteriaApi(params);
  ...
  function handleSidebarFilter(values: any) {
    filter.value = values;
    fetchInspectionCriteria({ ...values });
  }
</script>
