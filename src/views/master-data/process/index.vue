<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <ProcessSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      ...
      >
        <template #extra>
          <Space>
            <Button size="small" :href="`${BACKEND_URL}/excel/Process.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-processes`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import { deleteProcessApi, searchProcessApi } from '@/api/sys/process';
  ...
  import ProcessModal from './component/ProcessModal.vue';
  import ProcessSidebar from './component/ProcessSidebar.vue';
  ...
  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchProcess({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchProcess({ page: page.value, pageSize: size });
  }

  async function fetchProcess(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchProcessApi({
        page: page.value,
      ...
      let totalRecord = processes.value.length;
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteProcessApi(record?.id);
      if (result) {
        fetchProcess();
