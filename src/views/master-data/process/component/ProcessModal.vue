...
<script lang="ts" setup>
  ...
  import { createProcessApi, updateProcessApi } from '@/api/sys/process';
  ...
  async function handleSubmit() {
    try {
      ...
      if (!unref(isUpdate)) {
        // Force create
        const res: any = await createProcessApi(values);
        if (res) {
        ...
        // Force update
        const res: any = await updateProcessApi({ ...values, id: record.value?.id ?? null });
        if (res) {
          msg.success('Thao tác thành công');
          emit('success', {}); // Emit success
          closeModal();
