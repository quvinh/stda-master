<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <MaterialCategorySidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      ...
      >
        <template #extra>
          <Space>
            <Button size="small" :href="`${BACKEND_URL}/excel/material_categories_data.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-material-categories`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import MaterialCategoryModal from './component/MaterialCategoryModal.vue';
  import MaterialCategorySidebar from './component/MaterialCategorySidebar.vue';

  const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://127.0.0.1:8000';

  const materialCategories = ref<IMaterialCategory[]>([]);
  ...
  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchMaterialCategory({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchMaterialCategory({ page: page.value, pageSize: size });
  }

  async function fetchMaterialCategory(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchMaterialCategoryApi({
        page: page.value,
      ...
      let totalRecord = materialCategories.value.length;
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteMaterialCategoryApi(record?.id);
      if (result) {
        fetchMaterialCategory();
  ...
  async function handleExport(params: any) {
    try {
      const response = await exportMaterialCategoryApi(params);
