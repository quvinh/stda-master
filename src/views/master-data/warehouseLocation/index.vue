<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <WarehouseLocationSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
      ...
      >
        <template #extra>
          <Space>
            ...
            >
            <Button size="small" :href="`${BACKEND_URL}/excel/WareHouseLocation.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-inventory-locations`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import WarehouseLocationModal from './component/WarehouseLocationModal.vue';
  import WarehouseLocationSidebar from './component/WarehouseLocationSidebar.vue';
  import {
    deleteWarehouseLocationApi,
    exportWarehouseLocationApi,
    searchWarehouseLocationApi,
  ...
  const warehouseLocations = ref<IWarehouseLocation[]>([]);
  ...
  async function fetchWarehouseLocation(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchWarehouseLocationApi({
        page: page.value,
      ...
      let totalRecord = warehouseLocations.value.length;
  ...
  async function handleExport() {
    try {
      const response: any = await exportWarehouseLocationApi();
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteWarehouseLocationApi(record?.id);
      if (result) {
        fetchWarehouseLocation();
