<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <ProductOrderSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
      ...
      >
        <template #extra>
          <Space>
            ...
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-product-orders`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import ProductOrderSidebar from './components/ProductOrderSidebar.vue';
  ...
  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchProductOrder({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchProductOrder({ page: page.value, pageSize: size });
  }

  async function fetchProductOrder(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchProductOrderApi({
        page: page.value,
