...
<script lang="ts" setup>
  ...
  import { createWarehouseApi, updateWarehouseApi } from '@/api/sys/warehouse';
  ...
  async function handleSubmit() {
    try {
      ...
      if (!unref(isUpdate)) {
        // Force create
        const res: any = await createWarehouseApi(values);
        if (res) {
        ...
        // Force update
        const res: any = await updateWarehouseApi({ ...values, id: record.value?.id ?? null });
        if (res) {
          msg.success('Thao tác thành công');
          emit('success', {}); // Emit success
          closeModal();
