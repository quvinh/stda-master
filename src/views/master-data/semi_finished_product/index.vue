<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <SemiFinishedProductSidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      ...
      >
        <template #extra>
          <Space>
            <Button size="small" :href="`${BACKEND_URL}/excel/SemiFinishedProduct_data.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-SemiFinishedProducts`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  import { ISemiFinishedProduct } from '@/api/model/ISemiFinishedProduct';
  import { deleteMaterialApi, searchMaterialApi } from '@/api/sys/material';
  ...
  import SemiFinishedProductModal from './component/SemiFinishedProductModal.vue';
  import SemiFinishedProductSidebar from './component/SemiFinishedProductSidebar.vue';

  const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://127.0.0.1:8000';

  const SemiFinishedProducts = ref<ISemiFinishedProduct[]>([]);
  ...
  const [registerTable, { setPagination }] = useTable({
    dataSource: SemiFinishedProducts,
  ...
  onMounted(() => {
    fetchSemiFinishedProduct();
  });

  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchSemiFinishedProduct({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchSemiFinishedProduct({ page: page.value, pageSize: size });
  }

  async function fetchSemiFinishedProduct(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchMaterialApi({
        page: page.value,
        pageSize: pageSize.value,
        type: 2, // Type = 2 => bán thành phẩm
        ...filter.value,
        ...param,
      });
      // Set data
      SemiFinishedProducts.value = response.data || [];
      // Set pagination
      const pagination: IPagination = response?.pagination ?? null;
      let totalRecord = SemiFinishedProducts.value.length;
  ...
  function handleSidebarFilter(values: any) {
    filter.value = values;
    fetchSemiFinishedProduct({ ...values });
  }
</script>
