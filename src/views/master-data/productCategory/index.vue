<template>
  <div class="flex pt-1 h-full">
    ...
    <div class="hidden lg:block w-[270px] mr-1">
      <ProductCategorySidebar :filter="filter" @success="handleSidebarFilter" />
    </div>
    ...
    <div class="flex-1 w-[600px] bg-white flex flex-col h-full">
      ...
      >
        <template #extra>
          <Space>
            <Button size="small" :href="`${BACKEND_URL}/excel/ProductCategories.xlsx`">
              <Icon icon="ant-design:download-outlined" />
              File upload mẫu
            </Button>
            <Upload
              :action="`${BACKEND_URL}/api/v1/import-product-categories`"
              name="file"
              accept=".xlsx"
...
<script setup lang="ts">
  ...
  import ProductCategoryModal from './component/ProductCategoryModal.vue';
  import ProductCategorySidebar from './component/ProductCategorySidebar.vue';
  import { getToken } from '@/utils/auth';
  import {
    deleteProductCategoryApi,
    exportProductCategoryApi,
  ...
  function pageChange(p, pz) {
    page.value = p;
    pageSize.value = pz;
    fetchProductCategory({ page: p, pageSize: pz });
  }

  function pageSizeChange(_current, size) {
    pageSize.value = size;
    fetchProductCategory({ page: page.value, pageSize: size });
  }

  async function fetchProductCategory(param: any = {}) {
    try {
      loading.value = true;
      const response: any = await searchProductCategoryApi({
        page: page.value,
      ...
      // Set data
      productCategories.value = response.data?.map((item) => ({
        ...item,
        product_name: item.product_name,
      }));
      // Set pagination
      const pagination: IPagination = response?.pagination ?? null;
      let totalRecord = productCategories.value.length;
  ...
  async function handleDelete(record: Recordable) {
    try {
      const result = await deleteProductCategoryApi(record?.id);
      if (result) {
        fetchProductCategory();
  ...
  async function handleExport(params: any) {
    try {
      const response = await exportProductCategoryApi(params);
