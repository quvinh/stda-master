  ...
  >
    <div class="flex-1 flex flex-col h-[calc(100%-60px)]">
      ...
      <div class="flex-1 flex flex-col overflow-y-auto section-search-input pr-1">
        <Divider style="margin: 8px 0">Thời gian truy vấn</Divider>
        ...
        <FormItem label="Thông số máy" name="equipment_parameter_id">
          <Select
            v-model:value="formData.equipment_parameter_id"
            placeholder="Vui lòng nhập"
            :options="equipmentParameterOptions"
            :allowClear="true"
            :showSearch="true"
          />
        </FormItem>
...
<script setup lang="ts">
  ...
  import EquipmentParameterResouce from '@/api/resource/EquipmentParameterResouce';
  ...
  const equipmentParameterOptions = ref<ISelectOption[]>([]);
  ...
  onMounted(async () => {
    ...
    const [processResponse, equipmentParameterRes] = await Promise.allSettled([
      processStore.search(),
      EquipmentParameterResouce.getOptions(),
    ]);
    ...
    if (equipmentParameterRes.status === 'fulfilled') {
      const map = new Map<string, string>();
      equipmentParameterRes.value.forEach((item) => {
        map.set(item.label, String(item.value));
      });
      const options = Array.from(map).map(([label, value]) => ({ label, value }));
      equipmentParameterOptions.value = options;
    }
  });
