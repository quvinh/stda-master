  ...
  >
    <div class="flex-1 flex flex-col h-[calc(100%-60px)]">
      ...
      <div class="flex-1 flex flex-col overflow-y-auto section-search-input pr-1">
        ...
        <FormItem label="Ngày bảo dưỡng" name="maintenance_date">
          <DatePicker aria-placeholder="Ngày bảo dưỡng" v-model:value="formData.maintenance_date"
        /></FormItem>
        <Divider style="margin: 4px 0; font-size: 14px">Điều kiện truy vấn </Divider>
        <FormItem label="Loại công việc bảo trì" name="maintenance_plan_id">
          <Select v-model:value="formData.maintenance_plan_id" placeholder="Chọn loại công việc">
            <Option
              v-for="job in props.filter.maintenanceType"
              :key="job.maintenanceId"
              :value="job.maintenanceId"
              >{{ job.maintenanceCategory }}
            </Option>
          </Select>
        </FormItem>
        <FormItem label="Đơn vị sử dụng" name="maintenance_unit">
          <Input v-model:value="formData.maintenance_unit" placeholder="Nhập đơn vị sử dụng"
        /></FormItem>
...
<script setup lang="ts">
  ...
  const props = defineProps({
    filter: {
      type: Object as PropType<{
        ...
        maintenanceType: IEquipmentMaintenanceHistories[];
      }>,
      default: () => ({
        process: [],
        dates: [dayjs()],
  ...
  const formData = reactive<any>({
    ...
    maintenance_date: '',
    maintenance_plan_id: props.filter.maintenance_id,
    maintenance_unit: props.filter.maintenance_unit,
