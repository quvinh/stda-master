...
<script lang="ts" setup>
  ...
  import { getPQCLots } from '@/api/sys/quality';
  ...
  import InputNGPQCModal from './components/InputNGPQCModal.vue';
  ...
  import ScannerPQCModal from './components/ScannerPQCModal.vue';
  ...
  const fetchBottomLots = async () => {
    const response: ApiResponse<any> = await axiosInstance.get('v1/qc/get-pqc-lots');
    if (response.success && response.data) {
      dataBottom.value = response.data;
    }

    axiosInstance.get('v1/qc/pqc-oqc-overview', { params: { qc_type: 'pqc' } }).then((response) => {
  ...
  onMounted(() => {
    oiHeaderStore.setTitle('QUẢN LÝ PQC');
    fetchBottomLots();
  });
  ...
  async function handleCheck() {
    const response: any = await searchInspectionCriteriaApi({ type: 'pqc' });
  ...
  async function handleSuccess({ values, result }) {
    ...
    const response: ApiResponse = await axiosInstance.post('v1/qc/submit-final-pqc', {
  ...
  async function handleInputEnter(e: any) {
    try {
      ...
      loading.value = true;
      if (lot_id) {
        const response: any = await getPQCLots({ lot_id });
