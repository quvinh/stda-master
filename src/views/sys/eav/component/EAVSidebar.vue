<template>
  ...
  >
    <div class="flex-1 flex flex-col h-full pt-5">
      <!-- <FormItem label="Mã đơn vị tính" name="id">
        <Input v-model:value="formData.id" placeholder="Vui lòng nhập" />
      </FormItem> -->

      <FormItem label="Tên bảng" name="model">
        <Select
...
<script setup lang="ts">
  import { getEAVModelsApi } from '@/api/sys/eav';
  ...
  const modelOptions = ref<ISelectOption[]>([]);
  ...
  async function fetchEAVModels() {
    const res: any = await getEAVModelsApi();
    if (res) {
      modelOptions.value = res.map((item) => ({ label: item?.name, value: item?.name }));
    }
  }
