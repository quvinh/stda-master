    ...
    <Form ref="formItemRef" name="form_item" :model="itemForm" layout="vertical">
      ...
      <template v-for="(item, index) in itemForm.items" :key="item.key">
        <Row :gutter="16">
          ...
          <Col span="8">
            <FormItem
              :label="index === 0 ? 'Mã khách hàng' : ''"
              :name="['items', index, 'customer_id']"
              :rules="{
                required: true,
                message: 'Vui lòng nhập',
                trigger: 'change',
            ...
            >
              <Select
                v-model:value="item.customer_id"
...
<script lang="ts" setup>
  ...
  import { searchCustomerApi as searchCustomer } from '@/api/sys/customer';
  import { updateMaterialExportApi, createProductImportApi } from '@/api/sys/warehouse';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      if (data.record?.items) {
        itemForm.items = data.record.items?.map((item) => ({
  ...
  async function handleSubmit() {
    try {
      ...
      if (!unref(isUpdate)) {
        // Force create
        const response: any = await createProductImportApi({
  ...
  async function fetchCustomer(params: any = {}) {
    if (!params?.page) params.page = 1;
    if (!params?.pageSize) params.pageSize = 20;
    const response: any = await searchCustomer(params);
    if (response?.data) {
      customerOptions.value = (response.data || []).map((item: any) => ({
        label: item.id,
        value: item.id,
      }));
    }
  ...
  async function handlesearchCustomer(value: string | undefined) {
    clearTimeout(timer.value);
    timer.value = setTimeout(() => {
      fetchCustomer({ id: value });
    }, 500);
  }
</script>
