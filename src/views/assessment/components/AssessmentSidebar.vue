<template>
  <Form ref="formRef" :model="formData" @keypress.enter="handleSubmit" @submit="handleSubmit"
    class="bg-white pl-2 pr-2 flex flex-col h-full section-form border-1 border-gray-300" layout="vertical">
    <div class="flex-1 flex flex-col h-[calc(100%-60px)]">
      ...
      <div class="h-[calc(100%-125px)] overflow-auto">
        <Row>
          ...
          <div @click="handlechangeQuizz(quizz)"
      ...
      <div class="h-[120px] flex flex-col gap-1">
        ...
        <div class="h-[40px] border-1 border-gray-300 flex flex-row w-full">
          <div class="w-2.75/4 flex flex-row gap-1 justify-start items-center p-1">
            ...
            <small class="font-semibold text-gray-500">({{ formatNumber((remainQuestionNumber / questionNumber) * 100)
              }}%)</small>
          </div>
          <div class="w-1.25/4 border-l-1 border-l-gray-300 flex justify-center items-center">
    ...
    <div class="sticky bottom-0 left-0 right-0 bg-white pt-1 border-t border-gray-300 section-button">
